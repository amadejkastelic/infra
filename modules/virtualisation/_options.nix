# Plain NixOS module defining `virtualisation.{sharedMemoryFiles,hugepages}`
# options. Imported relatively by modules/virtualisation/default.nix (the
# `_`-prefix keeps import-tree from auto-loading it as a flake-parts module).
# Replaces lib/virtualisation.nix.
{
  lib,
  config,
  ...
...
in
{
  ...
  config.systemd.tmpfiles.rules = mapAttrsToList tmpfileEntry cfg.sharedMemoryFiles;
